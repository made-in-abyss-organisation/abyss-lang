...
| 6 | **Mobile layer** | Bind Skia; widget framework; `state` → re-render. `Counter` runs on a real Android device. ← *started: `component`/`state`/`render` now **execute** in the interpreter — `Counter()` mounts, `render(app)` walks the widget tree against live state, and calling a method (e.g. `app.increment()`) re-renders with the new state (`examples/counter_app.aby`). Now lowered to **both** backends — the native binary and the interpreter print byte-identical frames (`examples/counter_app.aby` is in the differential suite). The render target is a headless text tree for now; the Skia binding and on-device run are still TODO.* |
...
| 8 | **Platform & packaging** | Android (APK) first, then iOS (IPA, requires a Mac). ← *desktop delivery pipeline done: tagged releases build prebuilt `abyssc` binaries for macOS (universal), Windows (x64) & Linux (x64) (`.github/workflows/release.yml`). **Android native execution proven**: an Abyss program now cross-compiles via the NDK and **runs on the Android emulator** as a native binary — one command, `scripts/android_run.ps1` / `.sh` (`abyssc --emit-c` → NDK clang, ABI-matched + 16 KB-page aligned → `adb push` → run), output byte-identical to the interpreter. A graphical Skia surface, an APK wrapper, and iOS/IPA are still TODO.* |
